...
```bash
pip install -e ".[dev]"
cp .env.example .env          # add CALLE_API_KEY, and GEMINI_API_KEY (free) or LLM_API_KEY for the real extractor
...
shows the transcript as a document with the evidence span highlighted, the verdict with its
...
`.python-version` + `.vercelignore` drive the build. Replay Mode needs nothing. Live "call
yourself" on the hosted URL is switched on by setting `CALLE_API_KEY`, `GHOSTLINE_MODE=live`,
`GHOSTLINE_WEBHOOK_BASE`, `CALLE_WEBHOOK_SECRET`, `GEMINI_API_KEY` (free tier — or `LLM_API_KEY`
for Anthropic), and Upstash Redis creds (free tier) as project env vars. A `Dockerfile` is
